...
void WriteReg(uint16 reg, uint8 value) {
  ...
  if (reg >= 0x3000 && reg < 0x3040) {
    ...
    if (g_snes->hasGsu) {
      ...
      uint16_t gsu_entry_pc = gsu_get_reg(g_snes->gsu, 15);  // launch PC, before the run mutates it
      if (reg == 0x301f && gsu_is_running(g_snes->gsu)) {
        // Run the GSU to completion, but bound it: a correct render finishes in
        // well under this budget (max observed ~0.4M cycles). A non-terminating
        // render (e.g. a runaway polygon span-fill from a bad count) would
        // otherwise spin here forever and wedge boot, because the CPU busy-waits
        // on the SFR GO bit. If we hit the cap the GSU is stuck, so force a STOP
        // so the CPU falls through and boot proceeds.
        static long s_cap = -1;
        ...
        // GSU/coprocessor frame is done. The transfer of the rendered framebuffer
        // from Game Pak RAM to VRAM is, on some games, a timing-sensitive beam-
        // raced DMA the static recomp can't reproduce. Hand off to the game layer
        // (if registered) to HLE-present the finished framebuffer. The engine
        // itself stays game-agnostic — it knows nothing about which PC, which RAM
        // region, or which VRAM destination. NULL hook = faithful path.
        if (g_gsu_frame_done)
          g_gsu_frame_done(gsu_entry_pc);
      }
    }
    debug_server_on_reg_write(reg, value);
    return;
